...
export default function sitemap(): MetadataRoute.Sitemap {
  const now = new Date();
  const coreRoutes = ["about", "services", "team", "business", "contacts", "consultation", "cases", "publications", "reviews"];
  return [
    ...
    ...publications.map((item) => ({ url: getAbsoluteSiteUrl(`publications/${item.slug}`), lastModified: now, changeFrequency: "monthly" as const, priority: 0.65 })),
