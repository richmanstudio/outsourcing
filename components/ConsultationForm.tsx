...
const readField = (formData: FormData, key: FieldName): string => {
...
export function ConsultationForm() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const message = [
      "Здравствуйте! Хочу записаться на юридическую консультацию.",
      "",
      `Имя: ${values.name}`,
      `Телефон: ${values.phone}`,
      `Категория: ${values.topic}`,
      `Ситуация: ${values.message}`,
    ].join("\n");

    const url = `${siteConfig.links.whatsapp}?text=${encodeURIComponent(message)}`;
  ...
  return (
    <form className="consultation-form" onSubmit={handleSubmit} noValidate>
      <div className="field-row">
        <label className={`field${errors.name ? " invalid" : ""}`}>
          ...
          <input
            ...
            onChange={() => setErrors((value) => ({ ...value, name: undefined }))}
          />
        ...
        <label className={`field${errors.phone ? " invalid" : ""}`}>
          ...
          <input
            ...
            onChange={() => setErrors((value) => ({ ...value, phone: undefined }))}
          />
          <small className="field-error">{errors.phone}</small>
        </label>
      </div>

      <label className={`field${errors.topic ? " invalid" : ""}`}>
        ...
        <select
          ...
          onChange={() => setErrors((value) => ({ ...value, topic: undefined }))}
        >
          <option value="">Выберите направление</option>
          <option>Арбитраж / бизнес</option>
          <option>Наследство</option>
          <option>Семейный спор</option>
          <option>Жилищный / имущественный спор</option>
          <option>Другое</option>
        </select>
        <small className="field-error">{errors.topic}</small>
      </label>

      <label className={`field${errors.message ? " invalid" : ""}`}>
        <span>Кратко опишите ситуацию</span>
        <textarea
          name="message"
          rows={5}
          required
          placeholder="Что произошло, какие документы и сроки уже есть"
          aria-invalid={Boolean(errors.message)}
          onChange={() => setErrors((value) => ({ ...value, message: undefined }))}
        />
        <small className="field-error">{errors.message}</small>
      </label>

      <label className={`consent-check${errors.consent ? " invalid" : ""}`}>
        <input
          type="checkbox"
          name="consent"
          required
          aria-invalid={Boolean(errors.consent)}
          onChange={() => setErrors((value) => ({ ...value, consent: undefined }))}
        />
        <span>
      ...
      <p className="form-status" role="status" aria-live="polite">
        {errors.consent ?? status}
      </p>

      <button className="button button-dark button-full" type="submit">
        Продолжить в WhatsApp <span aria-hidden="true">↗</span>
      </button>
    </form>
  );
}
